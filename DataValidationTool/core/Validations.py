...
def readAndPushDataToQueue(sourceType, query, dataKey):
	...
	try:
		...
		print("******************************Records connected from " + str(
			dbType.name) + "******************************")
...
def pushDataToComparison():
	...
	while True:
		try:
			counter += 1
			dataNode = dataQueue.getFirstNode()
			tree.insert_dataNode(dataNode)
			if ((counter * sys.getsizeof(dataNode)) > 200000):
...
def getTypeCastedValue(val, checknum=False):
	...
	if type(val) == int:
		...
		val = datetime.datetime.strftime(val, globalConfig.getPropertyValueOrDefaultValue("dataformats", "datetime",
																						  "%Y-%m-%d %H:%M:%S.%f"))
		if "%Y-%m-%d %H:%M:%S.%f" == globalConfig.getPropertyValueOrDefaultValue("dataformats", "datetime",
																				 "%Y-%m-%d %H:%M:%S.%f"):
			val = str(val[:-3])
		return val
	elif type(val) == str:
...
def validateUI(windowShown: QtWidgets.QWidget):
	...
	print(config.getConfigSectionData("execution"))
	# Multiple Queries implementation
	reportsBaseFolder = str(config.getPropertyValue("execution", "resultsFolder")) + str(
	...
	for section in config.getSections():
		if "query" in str(section).lower():
			...
			threadExecSummary = lblTotalValidatedCount.text() + "\n" \
								...
								+ "Destination Query \t\t\t\t: " + str(
				sectionData.get("destinationquery")).strip() + "\n" \
			...
			updateJsonToUITable(windowShown, section.lower(), reportsFolder + "FIELDS_MISMATCH.json", "FIELDS_MISMATCH",
								int(config.getPropertyValueOrDefaultValue("execution", "maxresultrows", "1000"), 10))
			updateJsonToUITable(windowShown, section.lower(), reportsFolder + "MISSING_DATA.json", "MISSING_DATA",
								int(config.getPropertyValueOrDefaultValue("execution", "maxresultrows", "1000"), 10))
			updateJsonToUITable(windowShown, section.lower(), reportsFolder + "MATCHED.json", "MATCHED",
								int(config.getPropertyValueOrDefaultValue("execution", "maxresultrows", "1000"), 10))
	...
	if len(fileData) > 0:
		...
		for data in fileData:
			for key in data.keys():
				resultTable.setItem(iRowCounter, dataColumns.index(key.lower()),
									QtWidgets.QTableWidgetItem(str(data[key]).replace("|RED", "")))
				if "|RED" in data[key]:
					print(str(data[key]))
					resultTable.item(iRowCounter, dataColumns.index(key.lower())).setBackground(
						QtGui.QColor(255, 134, 134))
			iRowCounter += 1
